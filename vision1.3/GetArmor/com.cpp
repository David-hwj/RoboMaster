...
void SendDate(float x,float y,int fd){
    int sendx=(int) x;
    int sendy=(int) y;
...
//    sendata[0]=0xa2;
//    sendata[1]=0x03;
//    sendata[2]=0x04;//ycrc&0x00ff;//低八位
//    if(sendy<0) sendata[3]=('1'-'0')&0xff;
////    double2byte(&sendata[7-cy],abs(sendy));//发送Y
//    sendata[4]=((unsigned int)abs(sendy)>>16)&0xff;
//    sendata[5]=((unsigned int)abs(sendy)>>8)&0xff;
//    sendata[6]=(0xff&(unsigned int)abs(sendy));
//    sendata[7]=0xff;
//    //检查发送结果
//    for(int i=0;i<8;i++)
//        cout<<i<<" : "<<(0xff&sendata[i])<<"| ";
//    cout<<endl;
//    nr=write(fd,sendata,8);//发送数据
//    if(nr>0) printf("send data to uart success! nr=%d \n",nr);
//    else   printf("\n send data to uart fail!");
...
int openusb(){
    int fd,fcom;
    char port[]="/dev/ttyUSB1"; //打开usb通信串口
