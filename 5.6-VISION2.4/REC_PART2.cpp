...
Armor_Detector::Armor_Detector(int rather_show_img) 
{
...
void Armor_Detector::init(Mat src_img, Mat thres_img)
{
	...
	// 将轮廓拟合，进行下一步操作
	fitting_contours(thres_img);
	// 过滤结束后，将过滤结果进行装甲板拟合
	fitting_armor(thres_img);
	// 从匹配成功的装甲板中寻找最佳射击目标
	find_shoot_object(thres_img);
...
void Armor_Detector::fitting_contours(Mat img)
{
	...
	for (int i = 0; i < contours_2.size(); i++)
	{
		s_fitEllipse = fitEllipse(contours_2[i]);  // 二维点集的椭圆拟合
		s_minAreaRect = minAreaRect(contours_2[i]);// 二维点集的旋转矩形拟合
		s.angle = s_fitEllipse.angle;              // 最小椭圆的角度赋予旋转矩形
		s.center = s_fitEllipse.center;            // 最小椭圆的中心点赋予旋转矩形
	...
	else
	{
		...
		for (int i = 0; i < light_rect.size(); i++)
		{
			for (int j = i + 1; j < light_rect.size(); j++)
			{
				...
				double angle_middle = fabs(light_rect[i].angle + light_rect[j].angle) / 2;           // 与x轴正方向的夹角的均值
				double angle_diff = fabs(light_rect[i].angle - light_rect[j].angle);                 // 夹角差值
				...
				double X_diff = abs(light_rect[i].center.x - light_rect[j].center.x);                 // 纵坐标差值

				//cout << "对于拟合椭圆" << i << " " << j << endl;
				//cout << "判断旋转矩形的条件中" << endl;
				//cout << "纵坐标差值满足：" << (Y_diff < MH_diff * 2) << " Y_diff: " << Y_diff << " MH_diff" << MH_diff << endl;
				//cout << "横坐标差值满足：" << (X_diff < height_max * 7) << endl;
				//cout << "灯带的角度在规定范围内：" << (angle_diff < T_ANGLE_THRE || 180 - angle_diff < T_ANGLE_THRE180) << endl;//即亮灯带夹角的差值低于阈值
				//cout << "高度差小于高度和" << (height_diff / height_sum < T_HIGH_RAT) << endl;
				//cout << "宽度差小于宽度和" << (width_diff / width_sum < T_WHIDTH_RAT) << endl;
				//cout << "中心点构成的线与两个灯条都垂直" << ((angle_middle + center_angle > 165 && angle_middle + center_angle < 195) || (angle_middle + center_angle > -15 && angle_middle + center_angle < 15) || (angle_middle + center_angle > 80 && angle_middle < 100)) << endl;
				//cout << "灯条" << i << "角度" << light_rect[i].angle << " " << "灯条" << j << "角度" << light_rect[j].angle << " 角度均值" << angle_middle << " 中心点构成的线的角度" << center_angle << endl;
				if (
					...
					// 匹配成功
				{
					...
					//cout << "纵坐标差值小于一定比例装甲板的宽度" << (Y_diff < armor_width / 3) << endl;
					//// 纵坐标差值小于三分之一装甲板的宽度
					//if (Y_diff < armor_width / 3)
					{
						//	cout << "识别成功" << endl << endl;
						armor_FIRST.build1_No = i; // 该装甲板由第i与第j根灯柱构成
						...
						armor_SECOND.push_back(armor_FIRST); // 将找出的庄家的旋转矩形保存到vector
						//cout << "两灯柱连线与灯柱的角度差值:" << fabs(center_angle - angle_middle) << " 两灯柱角度均值:" << angle_middle << " 两灯柱角度差" << angle_diff << " 灯柱i的角度" << light_rect[i].angle << " 灯柱j的角度" << light_rect[j].angle << " 两灯柱连线的角度" << center_angle << endl;
...
void Armor_Detector::find_shoot_object(Mat img)
{
	...
	// calculate fps
	fps = 1 / (((double)getTickCount() / getTickFrequency()) - t);
	// cout << fps << endl;
	fpsString << fps;
	 cout << "fpsString " << fpsString.str() << endl;
	// draw fps
	putText(tmp, fpsString.str(), Point(5, 20), FONT_HERSHEY_SIMPLEX, 0.5, Scalar(255, 255, 255));
